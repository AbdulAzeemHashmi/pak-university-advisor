...
def scrape_usaid_scholarship_universities():
    ...
    if not page_html:
        raise RuntimeError("USAID/HEC source could not be retrieved; existing scholarship data was left unchanged.")
    ...
    if not usaid_list:
        raise RuntimeError("USAID/HEC source contained no matchable university records; refusing to overwrite the existing list.")
